...
namespace LeagueSharp.Common
{
    ...
    public class HealthPrediction
    {
        ...
        private static void SpellbookOnStopCast(Obj_AI_Base target, SpellbookStopCastEventArgs args)
        {
            if (target.IsValid && args.StopAnimation)
            {
                if (ActiveAttacks.ContainsKey(target.NetworkId))
                {
                    ActiveAttacks.Remove(target.NetworkId);
                }
            }
        }
