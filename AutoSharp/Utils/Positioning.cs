    ...
    internal static class Positioning
    {
        ...
        internal static void OnUpdate(EventArgs args)
        {
            ...
            if (Heroes.Player.UnderTurret(true))
            {
                ...
                if (turret.CountNearbyAllyMinions(800) < 3) { Orbwalker.OrbwalkTo(HeadQuarters.AllyHQ.RandomizePosition()); }
            }
        ...
        internal static void UseAutoSharpARAMPositioning()
        {
            ...
            var team = Heroes.AllyHeroes.Where(h => !h.IsMe && !h.IsDead && h.Distance(farthestAlly) < 500 && h.Position.CountEnemiesInRange(550) <= Utility.CountAlliesInRange(h.Position, 550)).ToList();
        ...
        internal static Paths AllyZone()
        {
            ...
            foreach (var pathList in teamPaths)
            {
                ...
                {
                    if (Utility.LSIsWall(new Vector2(path.X, path.Y)))
                    {
                        wall.Add(path);
                    }
                }
